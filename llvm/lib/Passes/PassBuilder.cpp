...
Error PassBuilder::parseModulePass(ModulePassManager &MPM,
  ...
  if (startsWithDefaultPipelineAliasPrefix(Name)) {
    ...
    if (CJPipeline) {
      ...
      // Reject oversized stack allocations whose size computation overflows,
      // target-independently (also on ARM, where CJIRVerifier is skipped).
      MPM.addPass(CJAllocationSizeCheck());
