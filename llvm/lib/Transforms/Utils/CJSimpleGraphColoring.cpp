...
InstrLiveIntervalConflictGraph::InstrLiveIntervalConflictGraph(
  ...
  for (auto U : Nodes) {
    for (auto V : Nodes) {
      if (U != V && LiveIntervalMap.count(U->Instr) &&
          LiveIntervalMap.count(V->Instr) &&
          FunctionVarLifeTimeResult::overlapLifeInterval(
              LiveIntervalMap[U->Instr], LiveIntervalMap[V->Instr])) {
...
template <typename GraphT> unsigned SimpleGraphColoring<GraphT>::color() {
  ColorMap.clear();
  ColorNum = 0;
  ...
  auto AdjacentNodesHaveDifferentColor = [this](NodeRef N,
                                                unsigned Color) -> bool {
    for (auto It = GraphTraits<GraphT>::child_begin(N);
         It != GraphTraits<GraphT>::child_end(N); ++It) {
  ...
  while (!UnColorNodes.empty()) {
    ...
    Temp.clear();
    std::sort(
        UnColorNodes.begin(), UnColorNodes.end(), [this](NodeRef A, NodeRef B) {
