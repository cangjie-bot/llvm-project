...
bool CJObjectReuseOpt::runImpl(Function &F, FunctionVarLifeTimeResult &LifeTimeResult, CJAAResult &CJAARes, MemorySSA &MSSA) {
  clear();
  ...
  adjustForReuseByLiveInterval(F, BitSizeToNoEscapeVals, LifeTimeResult, CJAARes, MSSA);
...
PreservedAnalyses CJObjectReuseOpt::run(Function &F, FunctionAnalysisManager &FAM) {
  ...
  if (runImpl(F, LifeTimeResult, CJAARes, MSSA)) {
...
void CJObjectReuseOpt::adjustForReuseByLiveInterval(Function &F, CandidateReuseMap &BitSizeToNoEscapeVals, FunctionVarLifeTimeResult &LifeTimeResult, CJAAResult &CJAARes, MemorySSA &MSSA) {
  ...
  auto CalculateLiveIntervalForCJVar = [&F, &LifeTimeResult, &CJAARes, &AAQIP, &MSSA] (CandidateReuseMap &InputMap) -> CJVarToLiveIntervalMap {
      ...
      for (auto *CJVar : VarArr) {
        ...
        for (auto &Instr : instructions(F)) {
          ...
          if (MA && (&Instr != CJVar)) {
            ...
            AliasResult AliasRes = CJAARes.bestAlias(CJVarLoc, MemoryLocation::getAfter(Selector), AAQIP);
            std::cout << "may alias start : " << std::endl;
            if (AliasRes == AliasResult::MayAlias) {
              Instr.dump();
            }
            std::cout << "may alias end : " << std::endl;
            if (AliasRes != AliasResult::NoAlias && AliasRes != AliasResult::MayAlias && LifeTimeResult.SSAVarToLiveInterval.count(Selector)) {
