...
inline static bool isTypeHasPadding(Type *AddrType, const DataLayout &DL) {
  auto *ST =
      dyn_cast<StructType>(AddrType->getNonOpaquePointerElementType());
  if (!ST) {
    return true;
  }
          ...
          for (size_t ReplacerIndex = 0; ReplacerIndex < VarArr.size() - 1;
            ...
            for (size_t BeReplacedIndex = ReplacerIndex + 1;
              ...
              if (ReuseAddr->getType() != BeReplacedAddr->getType() &&
                  (isTypeHasPadding(ReuseAddr->getType(), DL) ||
                   isTypeHasPadding(BeReplacedAddr->getType(), DL))) {
                continue;
              }
