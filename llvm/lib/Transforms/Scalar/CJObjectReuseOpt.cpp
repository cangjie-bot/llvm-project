  ...
  for (auto &I : instructions(F)) {
    if (auto *CB = dyn_cast<CallBase>(&I)) {
      ...
      if (Callee && Callee->getName().contains(CONSTRUCTOR_KEY_WORD)) {
        ...
        // we could not get the addr size
        if (dyn_cast<Constant>(TypeInfoArg)->getNumOperands() == 0) {
          continue;
        }
        auto &DL = F.getParent()->getDataLayout();
        Type *Ty = Addr->stripPointerCasts()->getType()->getNonOpaquePointerElementType();
        uint64_t BitSize = DL.getTypeAllocSize(Ty) * BYTE_BIT_WIDTH;
  ...
  auto CalculateLiveIntervalForCJVar = [&F, &LifeTimeResult, &AARes, &MSSA] (CandidateReuseMap &InputMap) -> CJVarToLiveIntervalMap {
    ...
    for (auto &[BitSize, VarArr] : InputMap) {
      for (auto *CJVar : VarArr) {
        ...
        // not mem instr
        // TODO: it is better to mimic llvm::getUnderlyingObject for collecting instr
