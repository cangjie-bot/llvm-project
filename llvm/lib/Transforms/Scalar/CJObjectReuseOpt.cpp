...
// #endif
  ...
  auto MemEquivalenceVarsTypeMatch = [this] (CallBase *Replacee, CallBase *Replacer) -> bool {
    auto &ReplacerMemEquivalence = this->CJVarToMemEquivalenceVars[Replacer];
    auto &BeReplacedMemEquivalence = this->CJVarToMemEquivalenceVars[Replacee];
    ...
    for (auto *V: BeReplacedMemEquivalence) {
      if (ReplacerTypeSet.count(V->getType()) == 0 &&
          !CastInst::castIsValid(Instruction::CastOps::BitCast, getUnderlyingObject(Replacer->getArgOperand(0)), V->getType())) {
        return false;
      }
    }
    return true;
