...
bool CJObjectReuseOpt::runImpl(Function &F, FunctionVarLifeTimeResult &LifeTimeResult, AliasAnalysis &AA, MemorySSA &MSSA, DominatorTree &DT, LoopInfo &LI) {
  ...
  return transformForMemReuse(DT, LifeTimeResult);
}
  ...
  for (auto &I : instructions(F)) {
    if (auto *CB = dyn_cast<CallBase>(&I)) {
      ...
      if (Callee && Callee->getName().contains(CONSTRUCTOR_KEY_WORD)) {
        ...
        uint64_t BitSize = getBitSizeFromCJTypeInfoValue(dyn_cast<Constant>(dyn_cast<Constant>(TypeInfoArg)->getOperand(0)));
        ...
        while(!UseQueue.empty()) {
          ...
          for (User *U : Elem->users()) {
            ...
            if (auto *Call = dyn_cast<CallBase>(U)) {
              if (auto *RP = getArgumentAliasingToReturnedPointer(Call, false); RP &&
                  HavePushed.count(U) == 0) {
              UseQueue.push(U);
              HavePushed.insert(U);
              }
          ...
          if (auto *Call = dyn_cast<CallBase>(Elem)) {
            if (auto *RP = getArgumentAliasingToReturnedPointer(Call, false); RP &&
                HavePushed.count(RP) == 0) {
            UseQueue.push(RP);
            HavePushed.insert(RP);
            }
          }
        }
// #ifndef NDEBUG
...
bool CJObjectReuseOpt::transformForMemReuse(DominatorTree &DT, FunctionVarLifeTimeResult &LifeTimeResult) {
  ...
  auto ReplaceUseBetweenCJValLifeTime = [&LifeTimeResult, &Changed] (Value *Replacee, Value *Replacer, CallBase *CJVar, CJVarToLiveIntervalMap &CJVarLifeTimeMap) {
    ...
    if (Replacer->getType() != Replacee->getType()) {
      IRBuilder<> IRB(CJVar);
      FinalReplacer = IRB.CreateBitCast(Replacer, Replacee->getType());
    }
      ...
      if (LifeTimeResult.InstrsToIndex[Instr] >= CJVarLifeTimeMap[CJVar].first &&
        ...
        for (unsigned Index = 0; Index < Instr->getNumOperands(); ++Index) {
          if (Instr->getOperand(Index) == Replacee) {
            Instr->setOperand(Index, FinalReplacer);
            Changed = true;
          }
        }
      }
  ...
  auto ReplaceValueForReuse = [&DT, &LifeTimeResult, &ReplaceUseBetweenCJValLifeTime] (ReuseMultiMap &InputMap, CJVarToLiveIntervalMap &CJVarLifeTimeMap) {
    for (auto &[BitSize, VarArr] : InputMap) {
      ...
      for (size_t ReplacerIndex = 0; ReplacerIndex < VarArr.size() - 1; ++ReplacerIndex) {
        if (HaveBeenReplacedIndex.count(ReplacerIndex) == 0) {
          for (size_t BeReplacedIndex = ReplacerIndex + 1; BeReplacedIndex < VarArr.size(); ++BeReplacedIndex) {
            ...
            auto *BeReplaceeCB = VarArr[BeReplacedIndex];
            if (DT.dominates(ReplacerCB->getParent(), BeReplaceeCB->getParent()) &&
                HaveBeenReplacedIndex.count(ReplacerIndex) == 0) {
              Value *ReuseAddr = ReplacerCB->getArgOperand(0);
              Value *BeReplaced = BeReplaceeCB->getArgOperand(0);
              // check whether the addr operand is definded by cast or cj.malloc.object
              if (isPointerCastFamily(ReuseAddr) && isPointerCastFamily(BeReplaced)) {
                ...
                ReplaceUseBetweenCJValLifeTime(ReplaceeCastOperand, ReplacerCastOperand, BeReplaceeCB, CJVarLifeTimeMap);
                ReplaceUseBetweenCJValLifeTime(BeReplaced, ReuseAddr, BeReplaceeCB, CJVarLifeTimeMap);
                HaveBeenReplacedIndex.insert(BeReplacedIndex);
              } else if (isCJMallocObj(ReuseAddr) && isCJMallocObj(BeReplaced)) {
                ReplaceUseBetweenCJValLifeTime(BeReplaced, ReuseAddr, BeReplaceeCB, CJVarLifeTimeMap);
                HaveBeenReplacedIndex.insert(BeReplacedIndex);
              }
            }
