...
bool CJObjectReuseOpt::runImpl(Function &F, FunctionVarLifeTimeResult &LifeTimeResult, AliasAnalysis &AA, MemorySSA &MSSA, DominatorTree &DT, LoopInfo &LI) {
  ...
  return transformForMemReuse(LifeTimeResult, F.getName().str());
}
...
bool CJObjectReuseOpt::transformForMemReuse(FunctionVarLifeTimeResult &LifeTimeResult, const std::string &name) {
  bool Changed = false;
