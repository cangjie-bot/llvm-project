...
// Collect the same bit size of object memory which are not escape by Ctor instruction
...
void CJObjectReuseOpt::adjustForReuseByLiveInterval(Function &F,
                                                    CandidateReuseMap &BitSizeToNoEscapeVals,
                                                    FunctionVarLifeTimeResult &LifeTimeResult,
                                                    BatchAAResults &AARes,
                                                    MemorySSA &MSSA) {
  ...
  auto CalculateLiveIntervalForCJVar = [this, &F, &LifeTimeResult, &AARes, &MSSA] (CandidateReuseMap &InputMap) -> CJVarToLiveIntervalMap {
    ...
    for (auto &[BitSize, VarArr] : InputMap) {
      ...
      for (auto *CJVar : VarArr) {
        ...
        for (auto &Instr : instructions(F)) {
          ...
          if (&Instr != CJVar) {
            ...
            if (AliasRes == AliasResult::MustAlias) {
              ...
              CJVarLiveInterval = FunctionVarLifeTimeResult::combineLifeInterval(CJVarLiveInterval,
        ...
        LLVM_DEBUG({
          llvm::dbgs() << *CJVar << "\n";
          llvm::dbgs() << "CJVarLiveInterval = {" << std::to_string(CJVarLiveInterval.first)
      ...
      for (auto *CB: SkipCB) {
        VarArr.erase(std::remove(VarArr.begin(), VarArr.end(), CB), VarArr.end());
      }
    }
  ...
  auto SplitNonOverlappingCJVar = [] (CandidateReuseMap &InputMap, ReuseMultiMap &OutputMap, CJVarToLiveIntervalMap &CJVarToLiveInterval) {
    // Split the CJ var in the fixed bitsize VarArr by disjoint live interval
    for (auto &[BitSize, VarArr] : InputMap) {
...
bool CJObjectReuseOpt::transformForMemReuse(DominatorTree &DT, FunctionVarLifeTimeResult &LifeTimeResult) {
  ...
  auto ReplaceUseBetweenCJValLifeTime = [&LifeTimeResult] (Value *Replacee, Value *Replacer, CallBase *CJVar,
  ...
  auto ReplaceValueForReuse = [this, &Changed, &DT, &LifeTimeResult,
                               &MemEquivalenceVarsTypeMatch, &ReplaceUseBetweenCJValLifeTime]
