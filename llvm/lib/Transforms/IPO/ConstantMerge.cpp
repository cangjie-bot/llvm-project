...
static CanMerge makeMergeable(GlobalVariable *Old, GlobalVariable *New) {
  if (!Old->hasGlobalUnnamedAddr() && !New->hasGlobalUnnamedAddr())
    return CanMerge::No;
  if (hasMetadataOtherThanDebugLoc(Old) && !CJGVMergeable(GV))
    return CanMerge::No;
  assert(!(hasMetadataOtherThanDebugLoc(New) && !CJGVMergeable(GV)));
...
static void replace(Module &M, GlobalVariable *Old, GlobalVariable *New) {
  ...
  assert((Old->hasLocalLinkage() || CJGVMergeable(GV)) &&
