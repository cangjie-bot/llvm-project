...
public:
    ...
    void print() {
//         for (auto &[Val, LifeTime] : SSAVarToLiveInterval) {
// #ifndef NDEBUG
//             std::cout<< " ========== ["<< std::to_string(LifeTime.first)  << ", " << std::to_string(LifeTime.second) << "] ========== "  <<std::endl;
//             Val->dump();
// #endif
//         }
    }

    static LiveInterval combineLifeInterval(LiveInterval &LHS, LiveInterval &RHS) {
...
private:
  ...
  void analysisLiveInterval(Function &F, FunctionVarLifeTimeResult &LifeTimeResult);
};
