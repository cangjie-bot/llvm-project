...
private:
  ...
  using CJVarToLiveIntervalMap =
      SmallDenseMap<Value *, FunctionVarLifeTimeResult::LiveInterval, 16>;
  using CJVarToMemEquivalenceVarsMap =
      SmallDenseMap<Value *, std::set<Value *>>;
  void collectNoEscapeValue(Function &F,
                            CandidateReuseMap &BitSizeToNoEscapeVals);
  void adjustForReuseByLiveInterval(Function &F,
                                    CandidateReuseMap &BitSizeToNoEscapeVals,
                                    FunctionVarLifeTimeResult &LifeTimeResult,
                                    BatchAAResults &AARes, MemorySSA &MSSA);
  bool transformForMemReuse(DominatorTree &DT,
                            FunctionVarLifeTimeResult &LifeTimeResult);
  bool isHeapZoneValue(Value *Val);
  bool runImpl(Function &F, FunctionVarLifeTimeResult &LifeTimeResult,
               AliasAnalysis &AA, MemorySSA &MSSA, DominatorTree &DT,
               LoopInfo &LI);
  void clearMap();
