...
public:
  ...
  operator=(const InstrWithLiveIntervalNode &) = delete;
...
template <> struct GraphTraits<InstrWithLiveIntervalNode *> {
  ...
  static NodeRef getEntryNode(InstrWithLiveIntervalNode *InstrNode) {
    return InstrNode;
  }
...
class InstrLiveIntervalConflictGraph {
  ...
  using ValueToLiveInterval =
      SmallDenseMap<Value *, FunctionVarLifeTimeResult::LiveInterval, 16>;

  NodeArrTy Nodes;

public:
  explicit InstrLiveIntervalConflictGraph(
      SmallVector<CallBase *, 4> &SmallSizeCB,
      ValueToLiveInterval &LiveIntervalMap);
...
template <>
struct GraphTraits<InstrLiveIntervalConflictGraph *>
    : public GraphTraits<InstrWithLiveIntervalNode *> {
...
template <>
struct GraphTraits<const InstrLiveIntervalConflictGraph *>
    : public GraphTraits<const InstrWithLiveIntervalNode *> {
  ...
  nodes_begin(const InstrLiveIntervalConflictGraph *ILICG) {
    return nodes_iterator(ILICG->begin());
  }
...
template <typename GraphT> class SimpleGraphColoring {
private:
...
public:
  ...
  unsigned color();
  SmallDenseMap<NodeRef, unsigned, 4> getColorMapRes() const {
    return ColorMap;
  }
