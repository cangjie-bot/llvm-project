; RUN: opt -cj-rewrite-statepoint -verify -S < %s | FileCheck %s
; RUN: opt -passes=cj-rewrite-statepoint,verify -S < %s | FileCheck %s

declare i8 addrspace(1)* @gc_call()

declare i32* @fake_personality_function()

define i8 addrspace(1)* @test(i1 %c) gc "cangjie" personality i32* ()* @fake_personality_function {
; CHECK-LABEL: @test(
entry:
  br i1 %c, label %gc_invoke, label %normal_dest

gc_invoke:
; CHECK: [[TOKEN:%[^ ]+]] = invoke token {{[^@]+}}@llvm.cj.gc.statepoint{{[^@]+}}@gc_call
...
; CHECK-NEXT: [[RET_VAL:%[^ ]+]] = call i8 addrspace(1)* @llvm.cj.gc.result.p1i8(token [[TOKEN]])
