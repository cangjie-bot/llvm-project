...
;CHECK:     MOVSDmr %stack.3, 1, $noreg, 0, $noreg, $xmm7 :: (store (s64) into %stack.3)
;CHECK:     %10:fr64 = COPY $xmm6
;CHECK:     %41:fr64 = COPY $xmm5
;CHECK:     %44:fr64 = COPY $xmm4
;CHECK:     MOVSDmr %stack.4, 1, $noreg, 0, $noreg, $xmm3 :: (store (s64) into %stack.4)
;CHECK:     %6:fr64 = COPY $xmm2
;CHECK:     %52:fr64 = COPY $xmm1
;CHECK:     %55:fr64 = COPY $xmm0
;CHECK:     %3:gr64 = COPY $rdi
;CHECK:     %67:fr64 = MOVSDrm_alt %fixed-stack.0, 1, $noreg, 0, $noreg :: (load (s64) from %fixed-stack.0)
;CHECK:     MOVSDmr %stack.7, 1, $noreg, 0, $noreg, %67 :: (store (s64) into %stack.7)
;CHECK:     %14:fr64 = MOVSDrm_alt %fixed-stack.1, 1, $noreg, 0, $noreg :: (load (s64) from %fixed-stack.1, align 16)
;CHECK:     %58:fr64 = MOVSDrm_alt %fixed-stack.2, 1, $noreg, 0, $noreg :: (load (s64) from %fixed-stack.2)
;CHECK:     %62:fr64 = MOVSDrm_alt %fixed-stack.3, 1, $noreg, 0, $noreg :: (load (s64) from %fixed-stack.3, align 16)
;CHECK:     MOV64mr %stack.0, 1, $noreg, 0, $noreg, %3 :: (store (s64) into %stack.0)
;CHECK:     ADJCALLSTACKDOWN64 0, 0, 0, implicit-def dead $rsp, implicit-def dead $eflags, implicit-def dead $ssp, implicit $rsp, implicit $ssp
;CHECK:     STATEPOINT 2882400000, 0, 0, target-flags(x86-plt) @blam, 2, 9, 2, 0, 2, 59, 2, 0, 2, 1, 2, 0, 2, 0, 2, 0, 2, 26, 2, 0, 2, 0, 1, 8, %stack.0, 0, 2, 4, %55, 2, 7, 2, 0, 2, 4, %52, 2, 7, 2, 0, 2, 4, %6, 2, 7, 2, 0, 2, 4, 1, 8, %stack.4, 0, 2, 7, 2, 0, 2, 4, %44, 2, 7, 2, 0, 2, 4, %41, 2, 7, 2, 0, 2, 4, %10, 2, 7, 2, 0, 2, 4, 1, 8, %stack.3, 0, 2, 7, 2, 0, 2, 4, %62, 2, 7, 2, 0, 2, 4, %58, 2, 7, 2, 0, 2, 4, %14, 2, 7, 2, 0, 2, 4, 1, 8, %fixed-stack.0, 0, 2, 7, 2, 0, 2, 7, 2, 0, 2, 1, 1, 8, %stack.0, 0, 2, 0, 2, 1, 0, 0, csr_64_mostregs, implicit-def $rsp, implicit-def $ssp :: (volatile load store (s64) on %stack.0), (load (s64) from %stack.3), (load (s64) from %stack.4), (load (s64) from %fixed-stack.0)
...
;CHECK:     MOVSDmr %stack.2, 1, $noreg, 0, $noreg, %44 :: (store (s64) into %stack.2)
;CHECK:     MOVSDmr %stack.5, 1, $noreg, 0, $noreg, %52 :: (store (s64) into %stack.5)
;CHECK:     MOVSDmr %stack.6, 1, $noreg, 0, $noreg, %55 :: (store (s64) into %stack.6)
;CHECK:     JCC_1 %bb.2, 4, implicit killed $eflags
;CHECK:   bb.1:
;CHECK:     successors: %bb.3(0x80000000)
;CHECK:     %49:fr64 = MOVSDrm_alt $rip, 1, $noreg, %const.0, $noreg :: (load (s64) from constant-pool)
;CHECK:     MOVSDmr %stack.3, 1, $noreg, 0, $noreg, %49 :: (store (s64) into %stack.3)
;CHECK:     MOVSDmr %stack.4, 1, $noreg, 0, $noreg, %49 :: (store (s64) into %stack.4)
;CHECK:     MOVSDmr %stack.7, 1, $noreg, 0, $noreg, %49 :: (store (s64) into %stack.7)
;CHECK:     JMP_1 %bb.3
;CHECK:   bb.2.bb13:
;CHECK:     successors: %bb.3(0x80000000)
...
;CHECK:     successors:
;CHECK:     ADJCALLSTACKDOWN64 8, 0, 0, implicit-def dead $rsp, implicit-def dead $eflags, implicit-def dead $ssp, implicit $rsp, implicit $ssp
;CHECK:     %68:fr64 = MOVSDrm_alt %stack.7, 1, $noreg, 0, $noreg :: (load (s64) from %stack.7)
;CHECK:     MOVSDmr $rsp, 1, $noreg, 0, $noreg, %68 :: (store (s64) into stack)
...
;CHECK:     %70:fr64 = MOVSDrm_alt %fixed-stack.2, 1, $noreg, 0, $noreg :: (load (s64) from %fixed-stack.2)
;CHECK:     $xmm7 = COPY %70
